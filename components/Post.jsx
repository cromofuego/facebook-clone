...
const Post = ({
  ...
  //when like update in the db update the likes in the app as wekk
  ...
  const likePost = async () => {
    //when we click once add like to fb
    //when we click doble clicked delete like from db
    if (hasLikes) {
  ...
  //When comment update in db update  them in the app as well
  useEffect(() => {
    onSnapshot(
      query(
  ...
  return (
    <div className='bg-white rounded-[1rem] px-5 py-4 mt-4' key={id}>
      {/* Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          ...
          <div className="ml-3">
            <p className='font-bold ml-4'>{username}</p>
            <div className="flex">
              <p className='text-xs'>
      ...
      <div className="div">
        <div className="flex justify-between text-[#8e8d8d] mt-1">
          <div className="flex items-center text-[#8e8d8d]">
            ...
            <p className='text pl-2 whitespace-nowrap text-[15px] sm:text-[16px]'>{`Emily Doe and another ${likes.length}`}</p>
          </div>
          <p className='whitespace-nowrap text-[15px] sm:text-[16px]'>{`${comments.length} Comments`}</p>
        </div>
        <div className="border-b my-2"></div>
        <div className="flex justify-between mx-6">
          <div className="flex items-center cursor-pointer" onClick={likePost}>
            <img src={hasLikes ? blueLike.src : unlike.src} className='w-6 h-6' />
      ...
      <div className="max-h-60 overflow-y-scroll">
        <div className="flex justify-between text-[#8e8d8d]">
          <p>{`See ${comments.length} previos comments`}</p>
          <div className="flex items-center">
            <p>Most Relevant</p>
            <RiArrowDownSLine />
          </div>
      ...
      <div className="flex items-center mt-4">
        ...
        <div className="w-full ml-2 bg-[#f2f3f7] rounded-full flex items-center relative">
          ...
          <div className="mr-4 bg-blue-400  hover:bg-blue-500/90 text-white rounded-full">
